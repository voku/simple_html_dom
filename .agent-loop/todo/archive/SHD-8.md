...
- **Summary:** The dogfood run exposed that scope-drift guidance returned to PLAN while an active governed Session first had to be closed/superseded. voku/agent-loop#152 shipped the explicit deterministic re-plan supersession handoff; this consumer now consumes that owner fix, and the card remains only as dogfood provenance.
- **Format version:** 1
